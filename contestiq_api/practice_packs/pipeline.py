    ...
    with store.connect() as conn:
        ...
        conn.execute(
            ...
            INSERT OR IGNORE INTO duel_problem_packs (
                ...
                mutation_score, test_count, activated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                pack["pack_id"],
...
def activate_oracle_packs(*, problem_ids: list[str] | None = None) -> dict[str, Any]:
    """Build and activate all registry packs that pass quality gates."""
    ...
    for problem_id in ids:
        ...
        try:
            ...
            continue
        if not pack.get("quality_report", {}).get("passed"):
            failed.append(
                {
                    "problem_id": problem_id,
                    "error": "quality gate failed: "
                    + ",".join(pack.get("quality_report", {}).get("failures") or []),
                }
            )
            continue
        with store.connect() as conn:
        ...
        if upsert_practice_pack(pack, activate=True):
            ...
            logger.info(
                "practice_pack_activated problem_id=%s pack_id=%s tests=%s mutation=%.3f",
                problem_id,
                pack["pack_id"],
                pack.get("test_count"),
